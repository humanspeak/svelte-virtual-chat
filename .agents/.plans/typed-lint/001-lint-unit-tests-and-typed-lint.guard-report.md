...
- **Plan amendments during execution:** one, applied _after_ this verdict with the operator's explicit agreement. **2026-07-09** — Step 2.5's claim that `stream-swap/+page.svelte:228` would trip `no-misused-promises` was removed and replaced with a "Not a violation — do not 'fix' it" callout; `Planned at` re-stamped `7257307` → `089bc10`; batch `README.md` status corrected `DONE` → `IN PROGRESS`. Rationale: the plan was wrong about reality (the defect case). Scope, `Why this matters`, and all eight done criteria are unchanged — the amendment deletes a false factual claim, it does not lower the bar, and the three failing criteria still fail.
...
4. ~~**Operator decision on the plan defect.**~~ **Done** — operator agreed 2026-07-09; the plan now records the gap and Step 2.5 no longer asks for a fix that no rule demands. Re-read the revised plan before resuming: the `stream-swap` line you were told to fix is explicitly marked _not_ a violation.
